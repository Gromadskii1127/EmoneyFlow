...
const Upload = ({ submitted, path, dispatch, intl, children, ...props }) => {
  ...
  const saveFileContent = useCallback((file) => {
    return new Promise(resolve => {
      ...
      dispatch(PayeeUploadCsvActions.saveData(path, newState));
    });
  }, [dispatch, path]);
  ...
  return (
    <Box>
      <Grid container >
        <Grid item md={2} />
        <Grid item md={8} xs={12} container spacing={2} className="m-3">
          <Grid item md={9} >
            <AdormentTextField
              fullWidth
              value={file?.name || ''}
